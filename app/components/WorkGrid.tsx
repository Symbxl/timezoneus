...
function Tile({ s, index }: { s: Series; index: number }) {
  ...
  return (
    <a
      href={s.href}
      className="tile group relative block overflow-hidden bg-[#0a0a0a] text-white border border-white/10 hover:border-[#0fa891]/70 transition-all duration-500"
    >
      <div className="flex flex-row items-stretch h-full min-h-[220px] md:min-h-[280px]">
        ...
        <div className="relative flex-1 flex flex-col justify-between p-6 md:p-8 overflow-hidden">
          <div className="flex items-start justify-between gap-3">
            <span className="display text-3xl md:text-4xl leading-none text-white/35 group-hover:text-[#0fa891] transition-colors">
              {num}
            </span>
          ...
          <div className="mt-auto pt-6">
            ...
            <div className="mt-5 md:mt-6 flex items-center gap-3 tag text-white/60 group-hover:text-[#0fa891] transition-colors">
          ...
          <span
            aria-hidden="true"
            className="absolute right-0 top-0 bottom-0 w-[2px] bg-[#0fa891] origin-top scale-y-0 group-hover:scale-y-100 transition-transform duration-500 ease-out"
          />
          {/* Faint number watermark in the bottom-right on hover */}
          <span
            aria-hidden="true"
            className="absolute -right-3 -bottom-12 display text-[12rem] leading-none text-[#0fa891]/0 group-hover:text-[#0fa891]/[0.06] transition-colors duration-700 select-none pointer-events-none"
          >
            {num}
          </span>
        </div>
